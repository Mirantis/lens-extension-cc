...
class ProviderStore {
  ...
  makeNew() {
    return {
      loading: false,
      // {boolean} true if currently loading data
      loaded: false,
      // {boolean} true if load is complete (regardless of error)
  ...
  clone() {
    ...
    return { ...this.store
    }; // assume only primitive props so no need to deep-clone.
  }
  /**
  ...
  triggerContextUpdate() {
    if (typeof this.setState !== 'function') {
      throw new Error('[ProviderStore.triggerContextUpdate()] setState() is not configured: Unable to trigger a context update');
    }
  ...
  notifyIfError() {
    if (this.store.error) {
      Notifications.error(`${this.store.error} ${_strings.noteOwner}`);
    }
  }
  /**
