...
class AuthAccess {
  ...
  constructor(spec) {
    DEV_ENV && rtv.verify({
      ...
      spec: [rtv.OPTIONAL, AuthAccess.specTs]
    });
    ...
    Object.defineProperty(this, 'changed', {
      ...
      // NOTE: we make this non-enumerable so it doesn't get persisted to JSON
      get() {
        return _changed;
      },
  ...
  isValid(passwordRequired = true) {
    const credsValid = passwordRequired ? this.hasCredentials() : !!this.username;
...
AuthAccess.tokensTs = {
  ...
  expires_in: [rtv.REQUIRED, rtv.SAFE_INT],
  // SECONDS valid from now
  refresh_token: [rtv.REQUIRED, rtv.STRING],
  refresh_expires_in: [rtv.REQUIRED, rtv.SAFE_INT],
  // SECONDS valid from now
...
AuthAccess.specTs = { ...Object.keys(AuthAccess.tokensTs).reduce((ts, key) => {
